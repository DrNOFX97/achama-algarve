...
For each inscrição still missing a signed PDF (`!pdfUrl`), the admin table offers two related but distinct actions: **"Copiar link de assinatura"** (mints a token, copies the resume URL to the clipboard, sends nothing) and **"Enviar documento para assinar"** (mints a token, regenerates the PDF from the inscrição's stored data, and actually emails it via `send-inscricao-docx.mjs` — for when the applicant never got the original email, or an old inscrição predates SMTP being configured).

Netlify Forms email notifications (`submission_created` → `geral@acimha.pt`, one hook per form: `inscricao`, `contacto`, `queixa-bairro`) are configured as **Netlify hooks via the API**, not in this repo — `netlify api` / the site's Forms dashboard is the only place to see or change them; grepping the codebase won't find them. **Deliberately not configured for `inscricao-assinatura`** — that form's completion is instead covered by the custom `send-assinatura-confirmacao.mjs` below, which sends a more useful pair of emails than Netlify's generic per-submission notice.

After the applicant uploads the signed PDF on `assinatura.html` (a plain Netlify Forms submission via `inscricao-signature.js`, no custom function involved in the upload itself), the success handler fires a **non-blocking** call to `netlify/functions/send-assinatura-confirmacao.mjs` with just the `token` from the resume-signature URL. That function re-derives the applicant's email/nome from the `inscricao-tokens` Blobs record (never trusts a client-supplied email, same pattern as `send-inscricao-docx.mjs`), best-effort enriches with categoria/telefone by searching the `inscricao` form's submissions by email (`lib/netlify-api.mjs`), and sends two emails via SMTP: a confirmation to the applicant, and a "pronto para aprovação" notice to `geral@acimha.pt` (hardcoded, distinct from `ALLOWED_ADMIN_EMAIL` which gates admin *login*) with a link to `/admin/`. If SMTP isn't configured or the call fails outright, the applicant's success screen is unaffected — `inscricao-signature.js` swallows the error, since the Netlify Forms submission (already completed) is the actual source of truth, not these emails.
